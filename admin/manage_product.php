...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			$res1=mysqli_query($con,"select id,categories from categories where id=$categories_id");
			$row1=mysqli_fetch_assoc($res1);
			$categories_name = $row1['categories'];
			$res2=mysqli_query($con,"select id,sub_categories from sub_categories where id=$sub_categories_id");
			$row2=mysqli_fetch_assoc($res2);
			$sub_categories = $row2['sub_categories'];
			if($_FILES['image']['name']!=''){
				$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
				move_uploaded_file($_FILES['image']['tmp_name'],"images/picture/".$image);
				$update_sql="update product set categories_id='$categories_id',categories_name='$categories_name',name='$name',mrp='$mrp',price='$price',qty='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword',image='$image',best_seller='$best_seller',sub_categories_id='$sub_categories_id',sub_categories='$sub_categories' where id='$id'";
			}else{
				$update_sql="update product set categories_id='$categories_id',categories_name='$categories_name',name='$name',mrp='$mrp',price='$price',qty='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword',best_seller='$best_seller',sub_categories_id='$sub_categories_id',sub_categories='$sub_categories' where id='$id'";
			}
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],"images/picture/".$image);
			mysqli_query($con,"insert into product(categories_id,categories_name,name,mrp,price,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,image,best_seller,sub_categories_id,sub_categories,added_by) values('$categories_id','$categories_name','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image','$best_seller','$sub_categories_id','$sub_categories','".$_SESSION['ADMIN_ID']."')");
		}
		header('location:product.php');
		die();
	}
